...
// This file is copied, with minor adaptations, from
...
// attribution.
//
// Adaptation: the upstream Config embeds k8sinventory.Config (Gvr, Namespaces,
// LabelSelector, FieldSelector, ResourceVersion) from the sibling
// internal/k8sinventory package. That package was not copied, so those fields
// are inlined directly into Config below instead of being embedded.
//
// Adaptation: upstream calls watch.NewRetryWatcherWithContext, added in a
// newer k8s.io/client-go than the v0.30.2 this repo's go.mod pins (contrib
// pins v0.35.4). Using watch.NewRetryWatcher (no ctx param) instead — cancellation
// still flows through since watchFunc's own resource.Watch(ctx, ...) call
// already carries ctx, and doWatch's select on stopperChan/ctx handles shutdown.
//
// Adaptation: upstream's getResourceVersion failure path called cancel()
// and gave up permanently on any error (e.g. a transient apiserver
// timeout), while wait.UntilWithContext's period of 0 means every other
// restart path (notably a routine 410 Gone, which is expected periodic
// behavior for any long-lived watch, not a failure) retries immediately
// with no delay. Giving up permanently on a transient error is worse than
// either extreme, so this now backs off (via internal/retry, shared with
// stream.go's reconnect loop) and keeps retrying instead — see the
// getResourceVersionBackoff* constants and their use in startWatch below.
// The 410 path is untouched: it must keep retrying with no delay, since
// treating it as a "failure" would make a healthy, hours-old watch climb
// an exponential backoff for no reason.
